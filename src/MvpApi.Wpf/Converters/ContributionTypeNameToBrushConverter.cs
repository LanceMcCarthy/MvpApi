...
namespace MvpApi.Wpf.Converters
{
    internal class ContributionTypeNameToBrushConverter : IValueConverter
    {
        ...
        // TODO IMPORTANT needs to be switched to guid ID as type names change over time
        public static SolidColorBrush GetForegroundColor(string contributionTypeEnglishName)
        {
            // PGIs do not have a area GUID
            if (contributionTypeEnglishName == "Product Group Interaction (PGI)")
            {
                return new SolidColorBrush(PaletteHelper.ContributionTypeBackgroundColors[1]);
            }

            switch (contributionTypeEnglishName)
            {
        ...
        public static SolidColorBrush GetBackgroundColor(string contributionTypeEnglishName)
        {
            if (contributionTypeEnglishName == "Product Group Interaction (PGI)")
            {
                return new SolidColorBrush(PaletteHelper.ContributionTypeBackgroundColors[1]);
