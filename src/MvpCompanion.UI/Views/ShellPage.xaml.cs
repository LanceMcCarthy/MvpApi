...
namespace MvpCompanion.UI.Views
{
    public sealed partial class ShellPage : PageBase
    {
        ...
        public async void WebView_OnNavigationCompleted(WebView2 sender, WebView2NavigationCompletedEventArgs e)
        {
                if (sender.BaseUri.AbsoluteUri.Contains("code="))
            {
                // get token
                var authCode = sender.BaseUri.ExtractQueryValue("code");

                var authorizationHeader = await RequestAuthorizationAsync(authCode);

                await InitializeMvpApiAsync(authorizationHeader);
            }
            else if (sender.BaseUri.AbsoluteUri.Contains("lc="))
            {
