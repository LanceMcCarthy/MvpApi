...
if grep -q "$iosSubstring" <<< "$tarFrmwk"; then
  echo "-------- ...using iOS simulator --------"
    ...
    dotnet build -t:Run -f "$tarFrmwk" -p:_DeviceName=:v2:udid=16E44511-85550-49DB-8BE2-A78177912809
else
    dotnet build -t:Run -f "$tarFrmwk"
fi
